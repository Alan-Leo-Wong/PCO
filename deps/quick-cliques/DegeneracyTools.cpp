...
*/
    ...
    NeighborList **computeDegeneracyOrderList(vector<list<int>> const &adjList, int size) {
        ...
        NeighborList **ordering = (NeighborList **) Calloc(size, (int)sizeof(NeighborList *));
        ...
        for (i = 0; i < size; i++) {
            ordering[i] = (NeighborList *) Malloc((int) sizeof(NeighborList));
        }
...
*/
    ...
    NeighborListArray **computeDegeneracyOrderArray(vector<list<int>> const &adjList, int size) {
        ...
        NeighborListArray **orderingArray = (NeighborListArray **) Calloc(size, (int) sizeof(NeighborListArray *));
        ...
        for (int currentVertexIndex = 0; currentVertexIndex < size; currentVertexIndex++) {
            ...
            for (int i = 0; i < splitPoint; ++i) {
                ...
                if (degree[neighbor] != -1) {
                    ...
                    if (firstIndexWithSameDegree == (size - 1) ||
                        degree[verticesOrderedByDegree[firstIndexWithSameDegree + 1]] != degree[neighbor]) {
    ...
    vector<NeighborListArray> computeDegeneracyOrderArrayForReverse(vector<vector<int>> &adjArray, int size) {
        ...
        while (numVerticesRemoved < size) {
            if (!verticesByDegree[currentDegree].empty()) {
                ...
                auto compareOrderNumber = [&vOrderingArray](int const left, int const right) {
                    return vOrderingArray[left].orderNumber < vOrderingArray[right].orderNumber;
                };
