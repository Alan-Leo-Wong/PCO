...
//            (mapCommandLineArgs.find("--input-file") != mapCommandLineArgs.end()) ? mapCommandLineArgs["--input-file"]
//                                                                                  : "");
//    string const algorithm(
//            (mapCommandLineArgs.find("--algorithm") != mapCommandLineArgs.end()) ? mapCommandLineArgs["--algorithm"]
//                                                                                 : "");
...
//        cout << "USAGE: " << argv[0] << " --input-file=<filename> --algorithm=<tomita|adjlist|degeneracy|hybrid>"
//             << endl;
//        return 1;
//    }
//
...
//        if (!bTableMode) cout << "Detected .graph extension, reading METIS file format.\n " << endl << flush;
//        adjacencyList = readInGraphAdjListEdgesPerLine(n, m, inputFile);
//        bOneBasedVertexIds = true;
//    } else {
//        if (!bTableMode) cout << "Reading .edges file format: one edge per line.\n " << endl << flush;
...
//        cout << "ERROR!: unable to compute adjacencyMatrix, since the graph is too large: " << adjacencyList.size()
//             << " vertices." << endl << flush;
//        exit(1);
//    }
//
...
//        adjacencyMatrix = (char **) Calloc(n, (int) sizeof(char *));
//        vAdjacencyMatrix.resize(n);
//
//        for (int i = 0; i < n; i++) {
//            adjacencyMatrix[i] = (char *) Calloc(n, (int) sizeof(char));
...
//    // Run algorithm
//    int maxCliqueSize = 0;
//    std::vector<std::list<int>> cliques;
//
...
//
//    RunAndPrintStats(pAlgorithm, bTableMode);
//
//    //std::cout << "cliques.size = " << cliques.size() << std::endl;
//    std::cout << "maxCliqueSize = " << maxCliqueSize << std::endl;
//    for (auto iter = cliques.rbegin(); iter != cliques.rend(); ++iter) {
//        if (iter->size() < maxCliqueSize) break;
