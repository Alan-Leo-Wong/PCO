...
namespace QuickClique {
    ...
    */
    ...
    DegeneracyAlgorithm::DegeneracyAlgorithm(vector<list<int>> const &adjacencyList)
            : Algorithm("degeneracy"), m_AdjacencyList(adjacencyList) {
    }

    DegeneracyAlgorithm::~DegeneracyAlgorithm() {
    }

    long DegeneracyAlgorithm::Run() {
    ...
    */
    ...
    inline int findBestPivotNonNeighborsDegeneracy(int **pivotNonNeighbors, int *numNonNeighbors,
        ...
        *pivotNonNeighbors = (int *) Calloc(beginR - beginP, (int)sizeof(int));
        ...
        *pNewBeginX = *pNewBeginP;
        ...
        while (j < orderingArray[orderNumber]->earlierDegree) {
            ...
            neighborsInP[neighbor] = (int *) Calloc(
                    min(*pNewBeginR - *pNewBeginP, orderingArray[neighbor]->laterDegree), (int) sizeof(int));
        ...
        while (j < *pNewBeginR) {
            ...
            neighborsInP[vertexInP] = (int *) Calloc(min(*pNewBeginR - *pNewBeginP,
                                                         ...
                                                         + orderingArray[vertexInP]->earlierDegree), (int) sizeof(int));
    ...
    */
    ...
    long
    DegeneracyAlgorithm::listAllMaximalCliquesDegeneracy(vector<list<int>> const &adjList, int size) {
        // vertex sets are stored in an array like this:
        // |--X--|--P--|
        int *vertexSets = (int *) Calloc(size, (int)sizeof(int));

        // vertex i is stored in vertexSets[vertexLookup[i]]
        int *vertexLookup = (int *) Calloc(size, (int)sizeof(int));

        int **neighborsInP = (int **) Calloc(size, (int)sizeof(int *));
        int *numNeighbors = (int *) Calloc(size, (int)sizeof(int));
...
////    clock_t clockStart = clock();
        ...
        while (i < size) {
            vertexLookup[i] = i;
            vertexSets[i] = i;
            neighborsInP[i] = (int *) Calloc(1, (int)sizeof(int));
            numNeighbors[i] = 1;
            i++;
        }
