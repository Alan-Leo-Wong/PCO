...
*/
    ...
    void RunAndPrintStats(Algorithm *pAlgorithm, bool const outputLatex) {
        fprintf(stderr, "Algorithm [%s]: ", pAlgorithm->GetName().c_str());
        fflush(stderr);

        clock_t start = clock();

        long const cliqueCount = pAlgorithm->Run();
...
*/
    ...
    void DescribeVertex(int const lineNumber, int *vertexSets, int *vertexLookup, int const size, int const vertex,
                        int const beginX, int const beginD, int const beginP, int const beginR) {
        int const vertexLocation(vertexLookup[vertex]);

        cout << lineNumber << ": vertex " << vertex << " is in position " << vertexLocation
             << (vertexSets[vertexLocation] == vertex ? "(consistent)" : "(inconsistent: " +
                                                                         to_string(vertexSets[vertexLocation]) +
                                                                         " is there)") << " in set ";
    ...
    void DescribeState(int const lineNumber, int *vertexSets, int *vertexLookup, int const size, int const beginX,
                       int const beginD, int const beginP, int const beginR) {
    ...
    void CheckConsistency(int const lineNumber, size_t const recursionNumber, int *vertexSets, int *vertexLookup,
                          int const size) {
        //if (recursionNumber > 2) return;
        //cout << recursionNumber << "1 is in position " << ver
        for (int i = 0; i < size; ++i) {
            if (vertexSets[vertexLookup[i]] != i) {
                cout << recursionNumber << "(line " << lineNumber << ") : inconsistency -- vertex " << i
                     << " is supposed to be in position " << vertexLookup[i] << " but vertex "
                     << vertexSets[vertexLookup[i]] << " is there." << endl;
            }
        }
    }

    void CheckReverseConsistency(int const lineNumber, size_t const recursionNumber, int *vertexSets, int *vertexLookup,
                                 int const size) {
        //if (recursionNumber > 2) return;
        //cout << recursionNumber << "1 is in position " << ver
        for (int i = 0; i < size; ++i) {
            if (vertexLookup[vertexSets[i]] != i) {
                cout << recursionNumber << "(line " << lineNumber << ") : inconsistency -- vertex " << vertexSets[i]
                     << " is supposed to be in position " << vertexLookup[vertexSets[i]] << " but it is in position "
                     << i << "." << endl;
            }
        }
    }
