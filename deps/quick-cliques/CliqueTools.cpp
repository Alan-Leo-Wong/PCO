...
namespace QuickClique {
    ...
    bool
    CliqueTools::IsMaximalClique(vector<vector<int>> &adjacencyArray, list<int> const &clique, bool const verbose) {
        ...
        for (int const vertex: clique) {
            for (int const otherVertex: clique) {
                ...
                if (!adjacencyMatrix[vertex][otherVertex]) {
                    cout << "clique test failed: " << vertex << " should not be in the same set with " << otherVertex
                         << endl << flush;
                    return false;
                }
            }
